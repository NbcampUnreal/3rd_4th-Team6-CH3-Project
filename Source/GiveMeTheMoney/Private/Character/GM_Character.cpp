...
void AGM_Character::UpdateMovementSpeed(float Speed)
{
	if (GetCharacterMovement())
	{
		GetCharacterMovement()->MaxWalkSpeed = Speed;		//이동 속도 업데이트
		//UE_LOG(LogTemp, Warning, TEXT("UpdateMovement! Speed : %f"), GetCharacterMovement()->MaxWalkSpeed)
	}
}
...
float AGM_Character::GetMaxHealth() const
{
	return MaxHealth;
}
...
void AGM_Character::StopMovement(bool IsStop)
{
	if (IsStop)
	{
		GetCharacterMovement()->StopMovementImmediately();	//현재 속도 제거
		GetCharacterMovement()->DisableMovement();	// 이동 차단
	}
	else
	{
		GetCharacterMovement()->SetMovementMode(MOVE_Walking);	// 이동 설정
	}
